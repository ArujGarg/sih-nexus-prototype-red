...
class RailwaySimulator {
    constructor() {
        ...
        this.trains = [];
        this.tracks = {
            A: { name: 'Track A', capacity: 5, color: '#3498db', occupancy: 0, trains: [] },
            B: { name: 'Track B', capacity: 5, color: '#2ecc71', occupancy: 0, trains: [] },
            C: { name: 'Track C', capacity: 4, color: '#e74c3c', occupancy: 0, trains: [] }
        };
        this.metrics = {
            throughput: 0,
            averageDelay: 0,
    ...
    loadInitialData() {
        ...
        const initialTrains = [
            {id: "T001", type: "Express", speed: 120, priority: "High", position: 0, track: "A", destination: 1000, status: "moving"},
            {id: "T002", type: "Passenger", speed: 80, priority: "Medium", position: 100, track: "B", destination: 500, status: "moving"},
            {id: "T003", type: "Freight", speed: 60, priority: "Low", position: 200, track: "C", destination: 1000, status: "moving"},
            {id: "T004", type: "Passenger", speed: 90, priority: "Medium", position: 0, track: "B", destination: 1000, status: "moving"},
            {id: "T005", type: "Express", speed: 110, priority: "High", position: 300, track: "A", destination: 500, status: "moving"}
        ];
    ...
    setupEventListeners() {
        ...
        if (addTrainBtn) addTrainBtn.addEventListener('click', () => this.showAddTrainModal());
        ...
        if (closeModalBtn) closeModalBtn.addEventListener('click', () => this.hideTrainModal());
        if (closeAddTrainModalBtn) closeAddTrainModalBtn.addEventListener('click', () => this.hideAddTrainModal());
        if (addTrainForm) addTrainForm.addEventListener('submit', (e) => this.handleAddTrain(e));
    ...
    highlightTrack(trackId, highlightType = 'decision') {
        ...
        if (trackElement) {
            ...
            trackElement.classList.remove('highlight-decision', 'highlight-conflict', 'highlight-optimization');
    ...
    createTrainElement(train) {
        const trackElement = document.querySelector(`[data-track="${train.track}"] .track-line`);
        if (!trackElement) {
    ...
    startSimulation() {
        if (!this.isRunning) {
            ...
            this.addAIDecision('NEXUS Rail AI activated - monitoring all train movements');
            
            // Start immediate AI demonstration
            setTimeout(() => {
                if (this.isRunning) {
                    this.highlightTrack('A', 'decision');
                    this.addAIDecision('Initial optimization scan on Track A - analyzing traffic patterns', 'A');
                }
            }, 2000);
        }
    }
    ...
    emergencyStop() {
        ...
        this.addAIDecision('Emergency stop activated - all trains halted for safety');
    ...
    resetSimulation() {
        ...
        this.addAIDecision('NEXUS Rail AI System reset - all trains and metrics cleared');
    }
    ...
    completeTrain(train) {
        ...
        this.addAIDecision(`Train ${train.id} completed journey to station at position ${train.destination}`, train.track);
    ...
    addRandomTrain() {
        ...
        this.addAIDecision(`Auto-generated ${trainData.type} train ${trainData.id} assigned to Track ${assignedTrack}`, assignedTrack);
    ...
    checkConflicts() {
        ...
        Object.keys(this.tracks).forEach(trackId => {
            const trackTrains = this.tracks[trackId].trains.filter(t => t.status === 'moving');
    ...
    makeAIDecision() {
        ...
        const decisions = [
            { text: `Optimizing track allocation for Track ${randomTrack} based on current traffic density`, track: randomTrack, type: 'optimization' },
            { text: `Analyzing train priorities on Track ${randomTrack} to maximize throughput`, track: randomTrack, type: 'optimization' },
            { text: `Adjusting signal timing for Track ${randomTrack} to reduce overall delay`, track: randomTrack, type: 'decision' },
            { text: `Monitoring capacity utilization on Track ${randomTrack}`, track: randomTrack, type: 'decision' },
            { text: `Implementing predictive scheduling for incoming trains on Track ${randomTrack}`, track: randomTrack, type: 'optimization' },
            { text: `Balancing service requirements on Track ${randomTrack}`, track: randomTrack, type: 'decision' }
        ];
        
        const randomDecision = decisions[Math.floor(Math.random() * decisions.length)];
        this.addAIDecision(randomDecision.text, randomDecision.track, randomDecision.type);
    ...
    optimizeTrainSpeeds() {
        this.trains.forEach(train => {
            if (train.status === 'moving') {
                ...
                const utilization = this.tracks[train.track].occupancy / this.tracks[train.track].capacity;
    ...
    getOriginalSpeed(type) {
        switch (type) {
            case 'Express': return 120;
            case 'Passenger': return 85;
            case 'Freight': return 60;
            default: return 80;
        }
    }
    ...
    addAIDecision(decision, trackId = null, decisionType = 'decision') {
        ...
        this.aiDecisions.unshift({ time: timestamp, decision, track: trackId, type: decisionType });
    ...
    updateDecisionLog() {
        ...
        if (logElement) {
            logElement.innerHTML = this.aiDecisions.slice(0, 8).map(decision => 
                `<div class="decision-entry">
                    <div class="decision-time">${decision.time}${decision.track ? ` - Track ${decision.track}` : ''}</div>
    ...
    updateMetrics() {
        ...
        this.metrics.throughput = hoursElapsed > 0 ? Math.round(this.metrics.trainsCompleted / hoursElapsed) : 0;
        
        // Calculate average delay
        const activeTrains = this.trains.filter(t => t.status === 'moving');
        if (activeTrains.length > 0) {
            const totalDelay = activeTrains.reduce((sum, train) => sum + train.delay, 0);
        ...
        this.metrics.efficiency = Math.max(75, Math.round(maxEfficiency - conflictPenalty - delayPenalty));
    }
    ...
    updateDisplay() {
        ...
        Object.keys(this.tracks).forEach(trackId => {
            ...
            const utilizationPercent = Math.round((track.occupancy / track.capacity) * 100);
            
            const utilEl = document.getElementById(`track-${trackId.toLowerCase()}-util`);
            const percentEl = document.getElementById(`track-${trackId.toLowerCase()}-percent`);
    ...
    updateTrainsList() {
        ...
        if (listElement) {
            listElement.innerHTML = this.trains.map(train => 
                `<div class="train-item" data-train-id="${train.id}" style="cursor: pointer;">
                    <div class="train-info">
                        <div class="train-id">${train.id}</div>
                        <div class="train-type">${train.type}</div>
                    </div>
                    <div class="train-status status status--${train.status === 'moving' ? 'success' : 'info'}">
                        ${train.status}
                    </div>
                </div>`
            ).join('');
    ...
    showTrainDetails(train) {
        ...
        details.innerHTML = `
            <div class="form-group">
                ...
                <div>${Math.round(train.position)} / ${train.destination} units</div>
            </div>
            <div class="form-group">
            ...
            <div class="form-group">
                ...
                <div class="status status--${train.priority === 'High' ? 'error' : train.priority === 'Medium' ? 'warning' : 'info'}">${train.priority}</div>
            </div>
            <div class="form-group">
            ...
            <div class="form-group">
                ...
                <div class="status status--${train.status === 'moving' ? 'success' : 'info'}">${train.status}</div>
            </div>
            <div class="form-group">
                <label class="form-label">Progress</label>
                <div>${Math.round((train.position / train.destination) * 100)}% complete</div>
            </div>
        `;
    ...
    handleAddTrain(e) {
        ...
        const trainDestination = parseInt(document.getElementById('new-train-destination').value);
        ...
        this.addTrain(trainData);
        this.addAIDecision(`New ${trainData.type} train ${trainData.id} added to Track ${trainData.track} with ${trainData.priority} priority`, assignedTrack);
        this.highlightTrack(assignedTrack, 'decision');
    ...
    assignOptimalTrack() {
        ...
        Object.keys(this.tracks).forEach(trackId => {
            const utilization = this.tracks[trackId].occupancy / this.tracks[trackId].capacity;
    ...
    addMultipleTrains(count, scenarioName) {
        ...
        this.addAIDecision(`Loaded ${scenarioName} scenario with ${count} additional trains`);
    }
    ...
    recordChartData() {
        ...
        Object.keys(this.tracks).forEach(trackId => {
            const utilization = (this.tracks[trackId].occupancy / this.tracks[trackId].capacity) * 100;
    ...
    updateCharts() {
        try {
            ...
            if (this.charts.utilization) {
                const currentUtilization = Object.keys(this.tracks).map(trackId => 
